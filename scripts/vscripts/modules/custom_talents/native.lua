...
local function addTalent(talentName, heroName, group)
...
for heroName, heroData in pairs(npc_heroes) do
	...
	if type(heroData) == "table" and not isChanged then
		for _,talentName in pairs(heroData) do
			if type(talentName) == "string" and (string.starts(talentName, "special_bonus_unique_") or string.starts(talentName, "special_bonus_unique")) then
