...
BOSS_DAMAGE_ABILITY_MODIFIERS = {
	...
	doom_bringer_infernal_blade = 15,
	winter_wyvern_arctic_burn = 25,
	freya_ice_cage = 10,
...
OUTGOING_DAMAGE_MODIFIERS = {
	...
	modifier_intelligence_primary_bonus = {
		multiplier = function(attacker, victim)
			if not attacker:FindModifierByName("modifier_intelligence_primary_bonus") then return 1 end
			if (victim:IsTrueHero() or victim:IsIllusion()) and (attacker:IsTrueHero() or attacker:IsIllusion()) then
				...
				if attackerInt <= victimInt then return 1 end

				if attackerInt > victimInt then
					diff = attackerInt / victimInt
					if diff >= INTELLECT_PRIMARY_BONUS_DIFF_FOR_MAX_MULT then
						return 1 + INTELLECT_PRIMARY_BONUS_MAX_BONUS * 0.01
					else
						diff = 1 - victimInt / attackerInt
						return 1 + INTELLECT_PRIMARY_BONUS_MAX_BONUS * 0.01 * diff
					end
				end
			end
		end
...
INCOMING_DAMAGE_MODIFIERS = {
	...
	modifier_intelligence_primary_bonus = {
		multiplier = function(attacker, victim)
			if not victim:FindModifierByName("modifier_intelligence_primary_bonus") then return end
